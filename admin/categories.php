...
// removing categories can be a lengthy process
        ...
        case 'update_product':
            ...
            $sql_data_array = array('products_quantity' => (int)tep_db_prepare_input($HTTP_POST_VARS['products_quantity']),
                ...
                'measurement_id' => (int)tep_db_prepare_input($HTTP_POST_VARS['measurement_id']),
...
if ($action == 'new_product') {
    ...
    $measurement_array = array(array('id' => '0', 'text' => TEXT_NONE));
    $measurement_query = tep_db_query("select measurement_id, measurement_name from " . TABLE_MEASUREMENT . " order by measurement_name");
    while ($measurement = tep_db_fetch_array($measurement_query)) {
        $measurement_array[] = array('id' => $measurement['measurement_id'],
            'text' => $measurement['measurement_name']);
    }
    $languages = tep_get_languages();
                ...
                <table border="0" width="100%" cellspacing="0" cellpadding="0">
                    <tr>
                        ...
                        <td class="pageHeading"
                            align="right"><?php echo tep_draw_separator('pixel_trans.gif', HEADING_IMAGE_WIDTH, HEADING_IMAGE_HEIGHT); ?></td>
                    </tr>
                </table>
            </td>
        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <table border="0" cellspacing="0" cellpadding="0">
                                    <tr>
                                        <td class="main"
                                            valign="top"><?php echo tep_image(tep_catalog_href_link(DIR_WS_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], '', 'SSL'), $languages[$i]['name']); ?>
                                            &nbsp;
                                        </td>
    ...
    $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.products_height, p.products_length, p.products_width, p.measurement_id  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
    $product = tep_db_fetch_array($product_query);
    ...
    for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
        ...
        if ($pInfo->products_date_available > date('Y-m-d')) {
            ...
            <tr>
                <td align="center"
                    class="smallText"><?php echo sprintf(TEXT_PRODUCT_DATE_AVAILABLE, tep_date_long($pInfo->products_date_available)); ?></td>
            </tr>
            <?php
        } else {
            ?>
            <tr>
                <td align="center"
                    class="smallText"><?php echo sprintf(TEXT_PRODUCT_DATE_ADDED, tep_date_long($pInfo->products_date_added)); ?></td>
            </tr>
            <?php
        }
        ?>
    ...
    <tr>
        <td align="right"
            class="smallText"><?php echo tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link($back_url, $back_url_params)); ?></td>
    </tr>
    </table>
    <?php
} else {
            ...
            <td>
                <table border="0" width="100%" cellspacing="0" cellpadding="0">
                    <tr>
                        ...
                        <td class="pageHeading"
                            align="right"><?php echo tep_draw_separator('pixel_trans.gif', 1, HEADING_IMAGE_HEIGHT); ?></td>
                        <td align="right">
                            <table border="0" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
...
// Get parent_id for subcategories if search
                                    ...
                                    <td class="dataTableContent"
                                        align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($categories['categories_id'] == $cInfo->categories_id)) {
...
// find out the rating average from customer reviews
                                    ...
                                    <td class="dataTableContent" align="center">
                                        ...
                                        ?></td>
                                    <td class="dataTableContent"
                                        align="right"><?php if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id)) {
                                            echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', '');
                                        } else {
                                    ...
                                    <td colspan="3">
                                        <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                            <tr>
                                                ...
                                                <td align="right"
                                                    class="smallText"><?php if (sizeof($cPath_array) > 0) echo tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link(FILENAME_CATEGORIES, $cPath_back . 'cID=' . $current_category_id));
                                                    if (!isset($HTTP_GET_VARS['search'])) echo tep_draw_button(IMAGE_NEW_CATEGORY, 'plus', tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_category')) . tep_draw_button(IMAGE_NEW_PRODUCT, 'plus', tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_product')); ?>
                                                    &nbsp;
                                                </td>
                                            </tr>
                                        </table>
